...
//....shared mem.../
    ...
    uint64_t *addr;
//S_IRWXU | S_IRWXG
    if ((fd = shm_open("/mymem-shared", O_RDWR, 0)) < 0) {
    	perror("shm_open failed");
        exit(1);
    }
  // Map the memory object
  addr = (uint64_t *)(mmap(0, SHMSZ, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0));
  if (addr == MAP_FAILED) {
     perror("mmap failed");
     exit(0);
  }
  ...
  close(fd);
	...
	for(counter = 0; counter < NDATA; counter++)
		*(addr + 8 * counter) = buff[counter];
    ...
    while (*addr != 0); //pc shouldn't be 0
        //usleep(1);
	// Detach the shared memory segment. 
	if (munmap (addr, SHMSZ) == -1){
		perror("munmap");
		exit(0);
		}
  counter = 0;
...
/*/....shared mem.../
    ...
    // Now put some things into the memory for the other process to read.
	for(counter = 0; counter < NDATA; counter++)
		*(shm + 8 * counter) = buff[counter];
	//Finally, we wait until the other process changes the first character of our memory to '*', indicating that it has read what  we put there.
    while (*shm != 0); //pc shouldn't be 0
        //usleep(1);
	// Detach the shared memory segment. 
  shmdt (shm);
  counter = 0;
...
/*/....socket v2.../
  ...
  //return 0;
	...
	static uint32_t 			n_bytes_h, t_mod_h, t_mod_n, n_bytes_n, file_size;
	static uint64_t numero;

	static int 				result, len, n, fd, flag, send_quit;
	typedef int SOCKET;
	static SOCKET 				s;

	static struct 				sockaddr_in saddr;
	static struct in_addr		sIPaddr;

	static char 				buf[BUFLEN], tx[BUFLEN], filename[BUFLEN];

	static fd_set 				read_set;
	static struct timeval 		tval;
	-------------
		uint16_t 			port;
	uint32_t 			n_bytes_h, t_mod_h, t_mod_n, n_bytes_n, file_size;
	uint64_t numero;

	int 				result, len, n, fd, flag, send_quit;
	typedef int SOCKET;
	SOCKET 				s;
	...
	char 				buf[BUFLEN], tx[BUFLEN], filename[BUFLEN];
...
/*/....socket samu.../
	...
	static struct timeval 		tval;
  	...
  	result = inet_aton("127.0.0.1", &sIPaddr);
  	if(result == 0){
	...
	//n = writen(s, buf, len);
{
    ...
    {
	nwritten = send(s, ptr, nleft, 0); //OCCHIO ai MSG_NOSIGNAL (da mettere sul server)
	if (nwritten <=0)
	    break;
	else
	{
