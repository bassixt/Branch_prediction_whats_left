...
void initTagePredictor(struct predictor *pred_str){ //initialise the tage
...
void endTagePredictor(double  *iCount, double  *tage_pred, struct predictor *pred_str){
	double MPKBr =((1.0 - *tage_pred / *iCount) * 1000.0);
  	double accurateTage = (*tage_pred * 100.0 / *iCount);
	FILE *scr = fopen(OUTPUT_FILE,"w");
